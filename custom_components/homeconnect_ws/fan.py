...
class HCFan(HCEntity, FanEntity):
    ...
    def is_on(self) -> bool:
        if (
            self.entity_description.key == "fan_hood"
            and self._runtime_data.appliance.active_program is None
        ):
            return False
    ...
    async def async_set_percentage(self, percentage: int) -> None:
        ...
        if self.entity_description.default_program is None:
            data = []
            for entity in self._speed_entities.values():
            ...
            await self._runtime_data.appliance.session.send_sync(
                Message(resource="/ro/values", action=Action.POST, data=data)
            )
            return
